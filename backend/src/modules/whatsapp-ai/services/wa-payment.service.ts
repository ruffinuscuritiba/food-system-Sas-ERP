...
export class WaPaymentService {
  ...
  private get backendUrl(): string {
    ...
    if (!url.startsWith('https://')) {
      this.log.warn(
        'BACKEND_URL is not HTTPS — Mercado Pago webhooks will NOT be delivered in production. Set BACKEND_URL=https://... in Render env vars.',
      );
    }
    return url;
  }
  ...
  async createPix(params: {
    ...
    if (!this.accessToken) {
      ...
      return {
        pixCopyPaste:
          '00020101021226580014BR.GOV.BCB.PIX0136mock-wa-pix-key5204000053039865802BR5925FOODSAAS WA TEST6009SAO PAULO62140510wa123456306314D0',
        pixQrcode: null,
  ...
  async createPaymentLink(params: {
    ...
    this.log.log(
      `WA Payment link created: prefId=${data.id} order=${params.orderId}`,
    );
