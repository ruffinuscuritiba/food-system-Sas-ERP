...
const PAY_OPTIONS: { key: PayMethod; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "CREDIT_CARD", label: "Crédito",  icon: <CreditCard size={16} /> },
  { key: "DEBIT_CARD",  label: "Débito",   icon: <CreditCard size={16} /> },
];

const ORDER_TYPES: { key: OrderType; label: string; icon: React.ReactNode }[] = [
  { key: "DINE_IN",  label: "Mesa",      icon: <Users size={15} /> },
  { key: "PHONE",    label: "Telefone",  icon: <Phone size={15} /> },
...
export default function PDVPage() {
  ...
  return (
    <div className="h-screen bg-slate-950 text-white flex flex-col overflow-hidden">
      ...
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <header className="bg-slate-900 border-b border-slate-800 px-5 h-14 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-green-500 rounded-lg flex items-center justify-center">
              <ShoppingCart size={16} className="text-white" />
            </div>
            <span className="font-black text-base tracking-tight">Frente de Caixa</span>
          </div>
          <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${cash?.isOpen ? "bg-green-500/10 text-green-400 border-green-500/20" : "bg-red-500/10 text-red-400 border-red-500/20"}`}>
            <span className="w-1.5 h-1.5 rounded-full bg-current animate-pulse" />
            {cash?.isOpen ? `Aberto · R$ ${Number(cash.balance).toFixed(2)}` : "Caixa Fechado"}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setShowCashModal(true)} className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 transition text-slate-300 font-medium">
            <DollarSign size={14} /> Caixa
          </button>
          {cash?.isOpen && (
            <button onClick={closeCash} className="text-xs px-3 py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 font-medium transition border border-red-500/20">
              Fechar
            </button>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* ── Col 1: Categorias ─────────────────────────────────────────────── */}
        <aside className="w-48 bg-slate-900 border-r border-slate-800 flex flex-col overflow-hidden shrink-0">
          <div className="px-4 py-3 border-b border-slate-800 shrink-0">
            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Categorias</p>
          </div>
          ...
          <div className="p-2 border-t border-slate-800 shrink-0">
            <button
              ...
              className="w-full bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-1.5"
            >
              🍕 Montar Pizza
            </button>
          </div>
          ...
          <div className="px-4 py-3 border-b border-slate-800 shrink-0">
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-slate-900 border border-slate-800 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white placeholder-slate-600 focus:outline-none focus:border-green-500/60 transition"
              />
            </div>
          </div>

          {/* Products list (Goomer-style horizontal cards) */}
          <div className="flex-1 overflow-y-auto p-4">
            {loadingProducts ? (
              <div className="flex items-center justify-center h-48 gap-3">
                <div className="w-6 h-6 border-3 border-green-500 border-t-transparent rounded-full animate-spin" />
                <p className="text-slate-500 text-sm">Carregando cardápio...</p>
              </div>
            ) : filtered.length === 0 ? (
                ...
                {filtered.map((product) => {
                  ...
                  return (
                    <div
                      ...
                      className={`relative bg-slate-900 border rounded-2xl overflow-hidden cursor-pointer transition group flex h-28 ${
                        inCart
                          ? "border-green-500/40"
                    ...
                    >
                      ...
                      <div className="flex-1 px-4 py-3 flex flex-col justify-between min-w-0">
                        <div className="min-w-0">
                          <p className="font-bold text-sm text-white leading-tight line-clamp-2 pr-1">{product.name}</p>
                          {product.description && (
                            <p className="text-slate-500 text-xs mt-1 line-clamp-1 leading-tight">{product.description}</p>
                          )}
                        </div>
                        <div className="flex items-center justify-between mt-1">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              title="Montar pizza"
                              className="w-7 h-7 rounded-lg bg-orange-500/20 hover:bg-orange-500/40 text-orange-400 flex items-center justify-center text-sm transition"
                            >
                              🍕
                            </button>
                            <button
                              onClick={(e) => { e.stopPropagation(); addToCart(product); }}
                              className={`w-7 h-7 rounded-lg flex items-center justify-center transition font-bold text-xs ${
                                inCart
                                  ? "bg-green-500 text-white"
                                  : "bg-slate-700 text-slate-300 hover:bg-green-500 hover:text-white"
                              }`}
                            >
                      ...
                      <div className="w-28 shrink-0 relative overflow-hidden">
                        ...
                        {inCart && (
                          <div className="absolute top-2 right-2 w-6 h-6 bg-green-500 rounded-full flex items-center justify-center text-xs font-black shadow-lg">
                            {inCart.quantity}
                          </div>
                        )}
                      </div>
        ...
        <aside className="w-80 bg-slate-900 border-l border-slate-800 flex flex-col shrink-0">
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
            {cart.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-36 gap-2 text-slate-700">
                <ShoppingCart size={32} />
                <p className="text-xs text-slate-600 text-center">Toque em um produto para adicionar</p>
              </div>
            ) : cart.map((item) => (
              <div key={item.cartKey} className="flex items-start gap-2.5 bg-slate-800 rounded-xl p-2.5">
                {item.flavors ? (
                  <div className="w-9 h-9 rounded-lg bg-orange-500/20 flex items-center justify-center text-lg shrink-0">🍕</div>
                ) : item.product.imageUrl ? (
                  <img src={item.product.imageUrl} alt={item.product.name} className="w-9 h-9 rounded-lg object-cover shrink-0" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-semibold line-clamp-2 leading-tight text-slate-100">{item.product.name}</p>
                  {item.notes && item.flavors && (
                    <p className="text-orange-400/80 text-[10px] mt-0.5 leading-tight">{item.notes}</p>
                  )}
                ...
                <div className="flex items-center gap-1 shrink-0">
                  ...
                  <button onClick={() => removeItem(item.cartKey)} className="w-6 h-6 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-400 flex items-center justify-center ml-0.5 transition">
                    <X size={11} />
                  </button>
                </div>
              </div>
          ...
          {/* Pagamento + total + ações */}
          <div className="px-4 pb-4 pt-3 border-t border-slate-800 space-y-3 shrink-0">
            <div className="flex items-center justify-between">
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Pagamento</p>
              <button
                onClick={() => setSplitMode(!splitMode)}
                className={`text-xs px-2.5 py-1 rounded-lg font-bold transition ${
                  splitMode ? "bg-purple-500/20 text-purple-400 border border-purple-500/30" : "bg-slate-800 text-slate-400 hover:text-slate-200"
                }`}
              >
            ...
            {!splitMode ? (
              ...
              <div className="space-y-2">
                ...
                {splits.length < 4 && (
                  <button onClick={addSplit} className="text-xs text-green-400 hover:text-green-300 flex items-center gap-1 transition">
                    <Plus size={12} /> Adicionar forma
                  </button>
                )}
                <div className={`flex justify-between text-xs font-bold px-3 py-2 rounded-xl ${splitOk ? "text-green-400 bg-green-500/10" : splitRemaining > 0 ? "text-orange-400 bg-orange-500/10" : "text-red-400 bg-red-500/10"}`}>
            ...
            {cart.length > 0 && (
              <div className="bg-slate-800 rounded-xl px-4 py-3 flex items-center justify-between">
                <div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">{cartCount} {cartCount === 1 ? "item" : "itens"}</p>
                  <p className="text-[10px] text-slate-500 mt-0.5">Total do pedido</p>
                </div>
            ...
            <div className="flex gap-2">
              {cart.length > 0 && (
                <button onClick={clearCart} className="w-11 h-11 rounded-xl bg-slate-800 hover:bg-slate-700 transition flex items-center justify-center text-slate-500 hover:text-red-400 shrink-0">
                  <Trash2 size={17} />
                </button>
              )}
              <button
                ...
                className="flex-1 h-11 bg-green-500 hover:bg-green-600 disabled:opacity-30 disabled:cursor-not-allowed transition rounded-xl font-black text-sm flex items-center justify-center gap-2"
              >
                {submitting
        ...
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-3">
          <div className="bg-slate-950 border border-slate-800 rounded-2xl w-full max-w-2xl flex flex-col max-h-[95vh] overflow-hidden shadow-2xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 shrink-0">
              <div>
                <h2 className="text-lg font-black">🍕 Montar Pizza</h2>
            ...
            <div className="overflow-y-auto flex-1">
              ...
              <div className="px-5 py-4 border-b border-slate-800">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">1. Tamanho</p>
              ...
              {/* 2. Borda */}
              <div className="px-5 py-4 border-b border-slate-800">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">2. Borda</p>
                <div className="flex flex-wrap gap-2">
                  {PIZZA_BORDERS.map((b) => (
                    <button key={b} onClick={() => setPizzaBorder(b)}
              ...
              {/* 3. Sabores */}
              <div className="px-5 py-4 border-b border-slate-800">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">3. Quantos Sabores?</p>
                <div className="grid grid-cols-4 gap-2">
                  {[{ n: 1, label: "Inteira", icon: "🍕" }, { n: 2, label: "Meio a Meio", icon: "½" }, { n: 3, label: "3 Sabores", icon: "⅓" }, { n: 4, label: "4 Sabores", icon: "¼" }].map(({ n, label, icon }) => (
                    <button key={n} onClick={() => changeFlavorParts(n)}
                      className={`flex flex-col items-center gap-1 py-2.5 rounded-xl font-bold text-xs transition border ${flavorParts === n ? "bg-orange-500 border-orange-500 text-white" : "border-slate-700 text-slate-400 hover:border-slate-500 hover:text-white"}`}
                    >
              ...
              <div className="px-5 py-4">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">4. Escolha os Sabores</p>
                    ...
                    return (
                      <button key={i} onClick={() => setPizzaActiveSlot(i)}
                        className={`shrink-0 flex items-center gap-2 px-3 py-2 rounded-xl border text-sm transition ${
                          pizzaActiveSlot === i ? "border-orange-500 bg-orange-500/10 text-white"
                          : selected ? "border-green-500/40 bg-green-500/10 text-green-400"
                      ...
                      >
                        ...
                        {selected && (
                          <button onClick={(e) => { e.stopPropagation(); setFlavorSlot(i, null); }} className="text-slate-500 hover:text-red-400 transition ml-1">
                            <X size={12} />
                          </button>
                        )}
                      </button>
                    ...
                    return (
                      <button key={p.id} onClick={() => pickFlavor(p)}
                        className={`relative text-left rounded-xl border transition overflow-hidden ${
                          isInSlot ? "border-orange-500 bg-orange-500/10"
                          : usedInOther ? "border-green-500/30 bg-green-500/5 opacity-70"
                      ...
                      >
                        ...
                        <div className="p-2">
                          ...
                          <p className="text-orange-400 text-xs font-black mt-1">R$ {Number(p.salePrice).toFixed(2)}</p>
                        </div>
                        {isInSlot && <div className="absolute top-1 right-1 w-5 h-5 rounded-full bg-orange-500 flex items-center justify-center text-white text-xs font-black">✓</div>}
                        {usedInOther && !isInSlot && <div className="absolute top-1 right-1 w-5 h-5 rounded-full bg-green-500/80 flex items-center justify-center text-white text-xs font-black">✓</div>}
                      </button>
                    );
                  })}
                </div>
              ...
              {flavorSlots.some(Boolean) && (
                <div className="bg-slate-900 rounded-xl px-4 py-3 mb-3 flex items-center justify-between">
                  <div className="min-w-0 mr-4">
                    <p className="text-xs text-slate-500 font-semibold">Pizza {pizzaSize}{pizzaBorder !== "Sem borda" ? ` · Borda ${pizzaBorder}` : ""}</p>
                    <p className="text-sm font-semibold mt-0.5 truncate">{flavorSlots.filter(Boolean).map((f) => f!.name).join(" + ")}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <p className="text-xs text-slate-500">Total</p>
                    <p className="text-xl font-black text-orange-400">R$ {Math.max(...flavorSlots.filter(Boolean).map((f) => Number(f!.salePrice))).toFixed(2)}</p>
                  </div>
                </div>
              )}
              <div className="flex gap-3">
                <button onClick={() => setShowFlavorModal(false)} className="px-5 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 transition font-semibold text-sm text-slate-300">Cancelar</button>
                <button onClick={confirmFlavors} disabled={!flavorSlots.some(Boolean)}
                  className="flex-1 bg-orange-500 hover:bg-orange-600 disabled:opacity-40 disabled:cursor-not-allowed transition py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2"
                >
      ...
      {showCashModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md space-y-5">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-black">Controle de Caixa</h2>
              <button onClick={() => setShowCashModal(false)} className="text-slate-400 hover:text-white transition"><X size={22} /></button>
            </div>
            ...
            {!cash?.isOpen ? (
              <div className="space-y-3">
                ...
                <button onClick={async () => { await openCash(); setShowCashModal(false); }}
                  className="w-full bg-green-500 hover:bg-green-600 transition py-3 rounded-xl font-black">
                  Abrir Caixa
                </button>
              </div>
            ) : (
      ...
      {orderDone && (
        <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-green-500/20 rounded-3xl p-10 text-center max-w-sm w-full">
            <div className="w-16 h-16 bg-green-500/10 rounded-full flex items-center justify-center mx-auto mb-4">
              <span className="text-4xl">✅</span>
            </div>
            <h2 className="text-3xl font-black text-green-400 mb-2">Pedido Enviado!</h2>
            <p className="text-slate-400 mb-1">Pedido <span className="text-white font-black">{orderDone}</span></p>
            <p className="text-slate-600 text-sm mb-8">Enviado para a cozinha e registrado no sistema.</p>
            <button onClick={() => setOrderDone(null)}
              className="w-full bg-green-500 hover:bg-green-600 transition py-3 rounded-xl font-black text-lg">
              Novo Pedido
            </button>
          </div>
        </div>
