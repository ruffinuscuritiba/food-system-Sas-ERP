...
const CUSTOMER_NAMES = [
  'Ana Silva','Carlos Mendes','Fernanda Costa','Roberto Lima','Juliana Santos',
  'Marcos Oliveira','Patrícia Rodrigues','Thiago Alves','Camila Ferreira','Diego Sousa',
  'Larissa Carvalho','Bruno Martins','Amanda Pereira','Lucas Ribeiro','Bianca Gomes',
  'Rafael Nascimento','Isabela Castro','Gustavo Araújo','Letícia Barbosa','Leonardo Nunes',
  'Vanessa Campos','Felipe Cardoso','Natália Moreira','Henrique Dias','Priscila Teixeira',
  'Rodrigo Pinto','Mariana Correia','Fabio Cunha','Tatiane Vieira','Anderson Monteiro',
  'Cristina Freitas','Renato Batista','Daniela Azevedo','Leandro Cavalcanti','Monica Ramos',
  'Eduardo Figueiredo','Simone Paiva','Vitor Lopes','Elaine Tavares','Paulo Melo',
  'Cláudia Machado','Jorge Nogueira','Aline Bezerra','Adriano Braga','Sandra Fonseca',
  'Caio Duarte','Viviane Medeiros','Marcos Rocha','Luciana Peixoto','Flávio Borges',
  'Bruna Monteiro','Sérgio Cavalcante','Denise Mota','Alexandre Freire','Regina Lima',
  'Vinícius Ramos','Débora Sousa','Willian Castro','Priscila Gomes','Antônio Ferreira',
  'Miriam Alves','Fábio Correia','Cecília Borges','Otávio Melo','Lívia Santos',
  'Flávio Teixeira','Vera Medeiros','Osmar Barbosa','Cíntia Rocha','Nilton Carvalho',
  'Rosana Cunha','Paulo Roberto Dias','Adriana Vieira','Marco Aurélio Costa','Flávia Nunes',
  'Reinaldo Lima','Sônia Campos','Eder Cardoso','Tatiana Moreira','Wilson Pereira',
  'Carmem Rodrigues','José Antônio Martins','Helena Castro','Cleber Araújo','Alice Ferreira',
  'Sandro Nascimento','Elaine Gomes','Márcio Ribeiro','Núbia Oliveira','Ronaldo Silva',
  'Gisele Mendes','Luís Henrique Costa','Inês Sousa','Claudinho Alves','Beatriz Lima',
  'Davi Santos','Elisa Carvalho','Rodrigo Freire','Tâmara Martins','Heraldo Borges',
  'Izabela Melo','Cássio Teixeira','Marta Monteiro','Felipe Duarte','Renata Barbosa',
  'Gilson Castro','Melissa Rocha','Ivo Cunha','Aparecida Vieira','Washington Gomes',
  'Elisângela Medeiros','Artur Correia','Mônica Sousa','Bruno Henrique Santos','Vera Lúcia Lima',
  'Fernando Cavalcante','Simone Freitas','Leandro Martins','Adriane Campos','Fabrício Ramos',
  'Solange Nunes','Augusto Ferreira','Carina Carvalho','Douglas Oliveira','Fátima Costa',
  'Tomás Mendes','Keila Ribeiro','Vanderlei Alves','Patrícia Moreira','Diogo Nascimento',
  'Camila Azevedo','Rômulo Pereira','Débora Rodrigues','Hélio Santos','Cristiane Dias',
  'Pâmela Lima','Rodrigo Borges','Joice Martins','Cláudio Ferreira','Márcia Silva',
  'Alexandre Costa','Elisa Monteiro','Jefferson Teixeira','Silvana Rocha','Emerson Melo',
  'Lara Barbosa','Caio Freitas','Fernanda Vieira','Saulo Castro','Aila Gomes',
  'Joel Ribeiro','Andreia Cunha','Marcos Mendes','Cris Cardoso','Iuri Araújo',
  'Patsy Lima','Murilo Sousa','Gleice Nascimento','Danilo Carvalho','Tainara Santos',
  'Rafael Moreira','Lorena Ferreira','Hugo Martins','Camille Oliveira','Reinaldo Costa',
  'Edna Freire','Celso Barbosa','Suely Ramos','Alisson Rodrigues','Fernanda Alves',
  'Tiago Medeiros','Geovana Lima','Evandro Nunes','Isadora Campos','Joel Ferreira',
  'Nathalia Pereira','Diego Borges','Andreia Sousa','Paulo Cunha','Carla Melo',
  'Rodrigo Teixeira','Fernanda Batista','João Victor Rocha','Cristina Vieira','Marcos Gomes',
]

const DRIVER_NAMES = [
  'Marcos Pereira (Moto)','José Carlos Santos (Carro)','Pedro Henrique Lima (Moto)',
  'Antonio Ferreira (Bicicleta)','Ricardo Borges (Moto)',
]

const ADDRESSES = [
  'Rua das Palmeiras, 123 — Jardim Primavera','Av. Brasil, 456 — Centro',
  'Rua São João, 789 — Vila Nova','Rua dos Ipês, 321 — Jardim América',
  'Av. Paulista, 1000 — Bela Vista','Rua da Saudade, 55 — Bairro Alto',
  'Rua das Flores, 200 — Parque das Nações','Av. das Acácias, 700 — Vila Jardim',
  'Rua do Comércio, 88 — Centro Histórico','Rua Independência, 444 — Vila Real',
  'Rua Tiradentes, 67 — Jardim Esperança','Av. Santos Dumont, 230 — Aeroporto',
  'Rua Getúlio Vargas, 180 — Centro','Rua das Orquídeas, 350 — Residencial Park',
  'Rua 7 de Setembro, 99 — Boa Vista',
]

const PAYMENT_METHODS = ['CASH','PIX','CREDIT_CARD','DEBIT_CARD','PIX','PIX','CREDIT_CARD'] // PIX mais comum
...
export class DemoVitrineService {
  ...
  private async populateTier(t: TierConfig) {
    ...
    if (!SAFE_DEMO_IDS.includes(cid)) throw new Error(`Not a demo company: ${cid}`)

    // 1. Clear all existing demo data
    await this.clearDemo(cid)

    // 2. Rename company to professional name
    await this.prisma.company.update({ where: { id: cid }, data: { name: t.companyName } })
    ...
    for (const catKey of t.catKeys) {
      const catId = catIds[catKey]; if (!catId) continue
      for (const p of PRODUCTS_MAP[catKey]) {
        const id = `${cid}-prod-${p.key}`
        await this.prisma.product.upsert({
          where:  { id },
          update: { name: p.name, salePrice: p.price, costPrice: p.cost, imageUrl: img(p.imgId), description: p.desc, categoryId: catId },
          create: { id, name: p.name, salePrice: p.price, costPrice: p.cost, imageUrl: img(p.imgId), description: p.desc, categoryId: catId, companyId: cid },
        })
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      const c = await this.prisma.customer.create({ data: { name: names[i], phone, companyId: cid } })
    ...
    // 7. Historical orders
    const histTotal = await this.createHistoricalOrders({ cid, products, customers, count: t.historyOrders, daysSpread: t.historyDays })

    // 8. Today's orders (to hit dashboard revenue target)
    const todayTotal = await this.createTodayOrders({ cid, products, customers, count: t.todayOrders, target: t.todayRevTarget })
  ...
  private async clearDemo(cid: string) {
    ...
    await safe(() => db.whatsappMessage.deleteMany({ where: { companyId: cid } }))
    await safe(() => db.whatsappConversation.deleteMany({ where: { companyId: cid } }))
    await safe(() => db.whatsappAiSettings.deleteMany({ where: { companyId: cid } }))
    await safe(() => db.whatsappConnection.deleteMany({ where: { companyId: cid } }))
    await safe(() => db.orderItemComplement.deleteMany({ where: { orderItem: { companyId: cid } } }))
    ...
    await safe(() => db.tableOrderItem.deleteMany({ where: { companyId: cid } }))
    await safe(() => db.tableOrder.deleteMany({ where: { companyId: cid } }))
    await this.prisma.financial.deleteMany({ where: { companyId: cid } })
    await safe(() => db.loyaltyAccount.deleteMany({ where: { companyId: cid } }))
    await this.prisma.customer.deleteMany({ where: { companyId: cid } })
    await safe(() => db.driverProfile.deleteMany({ where: { companyId: cid } }))
    await this.prisma.user.deleteMany({ where: { companyId: cid, role: 'DELIVERY' } })
  ...
  private async createHistoricalOrders(p: {
    cid: string; products: { id: string; salePrice: number; name: string }[];
    customers: { id: string; name: string; phone: string }[];
    count: number; daysSpread: number;
  }) {
    let total = 0
    for (let i = 0; i < p.count; i++) {
      ...
      total += await this.createOneOrder({ ...p, customer, createdAt, status: 'DELIVERED' })
    }
    return total
  }
  ...
  private async createTodayOrders(p: {
    ...
    for (let i = 0; i < p.count; i++) {
      ...
      const status    = i < recentCount ? pick(['CONFIRMED','PREPARING','READY']) : 'DELIVERED'
  ...
  private async createOneOrder(p: {
    ...
    const order = await this.prisma.order.create({
      data: {
        ...
        preparingAt:     ['PREPARING','READY','DELIVERED'].includes(p.status) ? ts(rnd(6,10)) : null,
        readyAt:         ['READY','DELIVERED'].includes(p.status) ? ts(rnd(22,28)) : null,
  ...
  private async setupWhatsappIa(cid: string, companyName: string) {
    ...
    await db.whatsappAiSettings.create({
      data: {
        ...
        offlineMessage:      'Olá! Estamos fora do horário agora, mas voltamos às 11h! 🍕 Deixa sua mensagem que te respondo assim que abrir.',
    ...
    const CONVOS = [
      {
        phone: '5511987654321', name: 'João Pedro',
        msgs: [
          { role: 'USER',      content: 'Oi, boa noite!' },
          { role: 'ASSISTANT', content: `Boa noite, João! 😊 Bem-vindo à ${companyName}! Sou a Carol. Tem alguma pizza em mente ou prefere que eu sugira a mais pedida aqui?` },
          { role: 'USER',      content: 'Me indica aí' },
          { role: 'ASSISTANT', content: 'Com certeza! 🍕 Nossa Pizza Calabresa é a mais pedida — calabresa artesanal, mussarela e cebola roxa. Vai ser para quantas pessoas?' },
          { role: 'USER',      content: 'Eu e minha esposa' },
          { role: 'ASSISTANT', content: 'Perfeito para dois! 😊 Uma grande já resolve bem. Quer adicionar uma Coca-Cola 2L por R$12,90? Fica ótimo junto.' },
          { role: 'USER',      content: 'Sim, pode adicionar' },
          { role: 'ASSISTANT', content: 'Feito! 🎉 Pedido: Pizza Calabresa Grande + Coca-Cola 2L. Total: R$62,80. Vai ser entrega ou retirada?' },
        ],
      },
      {
        phone: '5511976543210', name: 'Maria Clara',
        msgs: [
          { role: 'USER',      content: 'Vocês têm pizza de frango com catupiry?' },
          { role: 'ASSISTANT', content: `Sim! 🎉 Temos a Pizza Frango com Catupiry — frango desfiado temperado, catupiry original cremoso e mussarela. Muito boa! É pra quantas pessoas?` },
          { role: 'USER',      content: '4 pessoas' },
          { role: 'ASSISTANT', content: 'Para 4 recomendo uma pizza família! R$52,90 e abastece bem 😊 Vai querer adicionar bebida ou sobremesa?' },
          { role: 'USER',      content: 'Coloca uma Coca 2L e um brownie' },
          { role: 'ASSISTANT', content: '🤩 Pedido: Pizza Frango Catupiry Família + Coca-Cola 2L + Brownie com Sorvete. Total: R$76,70. Qual o endereço de entrega?' },
        ],
      },
      {
        phone: '5511965432109', name: 'Rafael Costa',
        msgs: [
          { role: 'USER',      content: 'Qual o tempo de entrega?' },
          { role: 'ASSISTANT', content: 'Olá, Rafael! 😊 Nossa estimativa é de 35 a 50 minutos dependendo do bairro. Posso te ajudar a montar seu pedido agora?' },
          { role: 'USER',      content: 'Quero ver o cardápio' },
          { role: 'ASSISTANT', content: 'Claro! Temos pizzas salgadas a partir de R$44,90, pizzas doces a partir de R$43,90, bebidas e combos especiais. Qual categoria te interessa mais?' },
        ],
      },
    ]
    ...
    for (const conv of CONVOS) {
      const c = await db.whatsappConversation.create({
        data: { connectionId: conn.id, companyId: cid, customerPhone: conv.phone, customerName: conv.name, status: 'ACTIVE', mode: 'AI' },
      })
      for (const msg of conv.msgs) {
        await db.whatsappMessage.create({
          data: { conversationId: c.id, companyId: cid, role: msg.role, content: msg.content, createdAt: new Date(Date.now() - rnd(5,120) * 60_000) },
        })
      }
    }
  }
  ...
  private async createFinancial(cid: string, days: number) {
    const EXPENSE_CATS = ['Ingredientes','Embalagens','Energia Elétrica','Mão de Obra','Aluguel','Marketing','Manutenção','Combustível']
