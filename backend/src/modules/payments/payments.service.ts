...
> = {
  BASIC: {
    amount: 9700,
    label: 'Plano Básico — FoodSaaS',
  },

  DELIVERY: {
    amount: 19700,
    label: 'Plano Profissional — FoodSaaS',
  },

  ENTERPRISE: {
    amount: 39700,
    label: 'Plano Enterprise — FoodSaaS',
  },
};
...
export class PaymentsService {
  ...
  private async createMercadoPagoCheckout(
    ...
    const body = {
      ...
      back_urls: {
        ...
        failure:
          dto.cancelUrl ||
          `${frontendUrl}/pagamento/cancelado`,
    ...
    if (!response.ok) {
      ...
      this.logger.error(
        `MercadoPago error: ${err}`,
      );

      throw new BadRequestException(
        'Erro ao criar checkout MercadoPago.',
      );
    }
  ...
  async handleWebhookMercadoPago(
    ...
    this.logger.log(
      `MercadoPago webhook: ${JSON.stringify(
        body,
      )}`,
    );
